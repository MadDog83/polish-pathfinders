...
const UA: Dict = {
  ...
  faq: {
    ...
    items: [
      ...
      { q: "Які документи потрібні для карти тимчасового побиту?", a: "Основний комплект: заповнена заява (подається електронно через портал MOS), актуальне цифрове фото, скан усіх сторінок дійсного паспорта, підтвердження сплати гербового збору та збору за карту побиту, а також документи, характерні для мети перебування (наприклад, для роботи — додаток від роботодавця та підтвердження медичного страхування). Відбитки пальців і зразок підпису знімають не при поданні заяви, а під час особистої явки, на яку викликає воєвода вже після подання заяви. Якщо не усунути формальні недоліки (включно з неявкою для зняття відбитків), заява залишиться без розгляду, і справа не вестиметься." },
      { q: "Через скільки років і за яку плату можна отримати постійне перебування?", a: "Підстави вичерпно перелічені в art. 195 ust. 1: власники чинної Карти Поляка з наміром оселитися; особи польського походження; діти громадян Польщі або діти іноземця з pobytem stałym / rezydenta UE, народжені після отримання дозволу; подружжя громадян Польщі — після 3 років шлюбу та 2 років безперервного перебування в Польщі на підставі пов’язаного з шлюбом дозволу на тимчасове перебування; жертви торгівлі людьми — після 1 року; особи з 5-річним перебуванням як біженці / додатковий захист / гуманітарні підстави, або 10-річним толерованим перебуванням. Гербовий збір — 640 злотих плюс 100 злотих за карту (власники Карти Поляка та особи зі статусом біженця / додаткового захисту / гуманітарної згоди звільнені від збору). Карта дійсна 10 років." },
      { q: "Що таке карта резидента довгострокового перебування ЄС і чим вона відрізняється від постійного перебування?", a: "Це окремий статус: надається на невизначений строк за наявності не менше 5 років безперервного легального проживання в Польщі, стабільного доходу, медичного страхування та підтвердженого знання польської мови не нижче рівня B1 (для дітей до 16 років мовна вимога не діє). Сама карта видається на 5 років і підлягає обміну — на відміну від 10-річної карти постійного перебування. Гербовий збір за дозвіл — 640 злотих, плюс 100 злотих за карту." },
      { q: "Як і через скільки років можна отримати польське громадянство?", a: "Є два окремі шляхи. «Визнання громадянином» — рішення воєводи: зазвичай після 3 років перебування на підставі постійного перебування, за стабільного доходу й правового титулу на житло (2 роки — для подружжя громадян Польщі, 1 рік — для осіб польського походження); гербовий збір — 1000 злотих, потрібне підтвердження мови на рівні B1. «Надання громадянства» — окреме дискреційне рішення Президента Польщі без фіксованих строків чи обов'язкових критеріїв, можливе навіть без виконання умов визнання воєводою; гербовий збір за це рішення — 1669 злотих." },
      { q: "Що таке карта побиту CUKR для громадян України?", a: "Карта побиту CUKR — це дозвіл на тимчасове перебування на 3 роки для осіб зі статусом UKR (на підставі тимчасового захисту, а не статусу біженця), які приїхали до Польщі після 23 лютого 2022. Заяви подаються електронно в порталі MOS. Вартість — 340 злотих плюс 100 злотих за карту. Чинний польський статутний дедлайн подачі — 4 березня 2027. Рамковий період тимчасового захисту ЄС окремо продовжено до 4 березня 2028, але це рішення ЄС ще не означає автоматичної зміни польського внутрішнього терміну. Дата може бути продовжена окремим польським актом, а може й ні — перевіряйте актуальний термін на gov.pl/UDSC ближче до дати." },
      ...
      { q: "Чи можу я виїхати з Польщі під час процедури?", a: "Так, але з обмеженнями. При тимчасовому перебуванні виїзд понад 6 місяців означає втрату дозволу. Для постійного перебування важливе безперервне перебування — одноразово поза Польщею не довше 6 місяців, сукупно не більше 10 місяців. Окремо для осіб зі статусом UKR (тимчасовий захист): виїзд з Польщі на строк понад 30 днів є самостійною підставою для згасання тимчасового захисту — це значно коротший строк, ніж для власників дозволів на перебування." },
...
const EN: Dict = {
  ...
  faq: {
    ...
    items: [
      { q: "How long does it take and how much does a temporary residence card cost?", a: "The statutory review period is 60 days from submitting the complete set of documents, and printing the card takes another 3-4 weeks; in practice it can take longer. The stamp duty is 440 PLN for the \"residence and work\" basis, and 340 PLN for other grounds, plus 100 PLN for the card itself (a reduced 50 PLN fee for students, pupils, and children under 16). You can track the status in the inPOL portal.\n" },
      ...
      { q: "What documents are needed for a temporary residence card?", a: "The basic package is: a completed application (submitted electronically via the MOS portal), a current digital photo, a scan of every page of a valid passport, proof of payment of the stamp duty and the card fee, and documents specific to your purpose of stay (for example, for work — the employer's attachment and proof of health insurance). Fingerprints and a signature sample are not taken when you submit the application, but later, during an in-person appointment the voivode calls you in for after the application is filed. If formal deficiencies are not remedied (including failing to appear for fingerprinting), the application is simply left unprocessed and the case does not proceed." },
      { q: "After how many years, and for what fee, can I get permanent residence?", a: "The grounds are exhaustively listed in art. 195 ust. 1: holders of a valid Polish Card with intent to settle; people of Polish origin; children of Polish citizens or children of a foreigner with permanent residence/EU resident status born after the permit was granted; spouses of Polish citizens after 3 years of marriage and 2 years of uninterrupted residence in Poland on a temporary residence permit connected to the marriage; victims of human trafficking after 1 year; and people with 5 years' residence as refugees, under subsidiary protection or humanitarian consent, or 10 years under tolerated stay. The stamp duty is PLN 640 plus PLN 100 for the card (Polish Card holders and persons with refugee/subsidiary-protection/humanitarian-consent status are exempt). The card is valid for 10 years." },
      { q: "What is the EU long-term resident card, and how does it differ from permanent residence?", a: "It's a separate status: granted for an indefinite period once you have at least 5 years of continuous legal residence in Poland, a stable income, health insurance, and confirmed Polish language proficiency at B1 level or higher (this language requirement doesn't apply to children under 16). The card itself is issued for 5 years and must be renewed — unlike the 10-year permanent residence card. The stamp duty for the permit is PLN 640, plus PLN 100 for the card." },
      { q: "How, and after how many years, can I obtain Polish citizenship?", a: "There are two separate paths. 'Recognition as a citizen' is a voivode's decision: usually after 3 years of residence based on permanent residence, with a stable income and legal title to housing (2 years for spouses of Polish citizens, 1 year for people of Polish origin); the stamp duty is PLN 1000, and B1-level language proficiency must be confirmed. 'Granting of citizenship' is a separate, discretionary decision by the President of Poland, with no fixed deadlines or mandatory criteria — possible even without meeting the voivode's recognition requirements; the stamp duty for this decision is PLN 1669." },
      { q: "What is the CUKR residence card for Ukrainian citizens?", a: "The CUKR residence card is a 3-year temporary residence permit for people with UKR status (based on temporary protection, not refugee status) who arrived in Poland after 23 February 2022. Applications are filed online in the MOS portal. The cost is PLN 340 stamp duty plus PLN 100 for the card. Poland's own statutory filing deadline is 4 March 2027. The EU's temporary-protection framework has separately been extended to 4 March 2028, but that EU-level decision does not automatically move Poland's domestic deadline. Poland may or may not extend its own deadline further — check gov.pl/UDSC closer to the date for the current rule." },
      ...
      { q: "Can I leave Poland during the procedure?", a: "Yes, but with limits. Under a temporary residence permit, leaving for more than 6 months means losing the permit. For permanent residence, uninterrupted residence counts — no single absence longer than 6 months, and no more than 10 months in total. Separately, for people with UKR status (temporary protection), leaving Poland for more than 30 days is in itself a ground for the temporary protection to expire — a much shorter limit than for residence-permit holders." },
...
const PL: Dict = {
  ...
  faq: {
    ...
    items: [
      { q: "Ile czasu zajmuje i ile kosztuje karta pobytu czasowego?", a: "Ustawowy termin rozpatrzenia wniosku to 60 dni od złożenia kompletu dokumentów, a wydrukowanie karty zajmuje kolejne 3-4 tygodnie; w praktyce bywa dłużej. Opłata skarbowa wynosi 440 złotych dla podstawy „pobyt i praca\", a 340 złotych dla pozostałych podstaw, plus 100 złotych za samą kartę (ulga 50 złotych dla studentów, uczniów i dzieci do 16 roku życia). Status śledzisz w portalu inPOL." },
      ...
      { q: "Jakie dokumenty są potrzebne do karty pobytu czasowego?", a: "Podstawowy komplet to: wypełniony wniosek (elektronicznie, przez portal MOS), aktualne zdjęcie cyfrowe, skan wszystkich stron ważnego paszportu, potwierdzenie opłaty skarbowej i opłaty za kartę pobytu oraz dokumenty właściwe dla celu pobytu (np. przy pracy — załącznik pracodawcy i potwierdzenie ubezpieczenia zdrowotnego). Odciski palców i wzór podpisu pobiera się nie przy składaniu wniosku, lecz podczas osobistego stawiennictwa, na które wzywa wojewoda już po złożeniu wniosku. Jeśli nie uzupełnisz braków formalnych (w tym niestawienie się na pobranie odcisków), wniosek pozostanie bez rozpoznania, a sprawa nie będzie prowadzona." },
      { q: "Po ilu latach i za jaką opłatą można otrzymać pobyt stały?", a: "Podstawy są wyczerpująco wymienione w art. 195 ust. 1: posiadacze ważnej Karty Polaka z zamiarem osiedlenia się; osoby polskiego pochodzenia; dzieci obywateli Polski lub dzieci cudzoziemca z pobytem stałym / rezydentem UE urodzone po uzyskaniu zezwolenia; małżonkowie obywateli Polski — po 3 latach małżeństwa i 2 latach nieprzerwanego pobytu w Polsce na podstawie zezwolenia na pobyt czasowy związanego z tym małżeństwem; ofiary handlu ludźmi — po 1 roku; oraz osoby po 5 latach pobytu jako uchodźcy / ochrona uzupełniająca / zgoda na pobyt z przyczyn humanitarnych, albo po 10 latach pobytu tolerowanego. Opłata skarbowa wynosi 640 zł plus 100 zł za kartę (posiadacze Karty Polaka oraz osoby ze statusem uchodźcy / ochrony uzupełniającej / zgody na pobyt z przyczyn humanitarnych są zwolnione z opłaty). Karta jest ważna 10 lat." },
      { q: "Czym jest karta rezydenta długoterminowego UE i czym różni się od pobytu stałego?", a: "To odrębny status: udzielany na czas nieoznaczony, gdy spełnione są łącznie co najmniej 5 lat nieprzerwanego legalnego pobytu w Polsce, stabilny dochód, ubezpieczenie zdrowotne i potwierdzona znajomość języka polskiego na poziomie co najmniej B1 (wymóg językowy nie dotyczy dzieci poniżej 16 roku życia). Sama karta jest wydawana na 5 lat i podlega wymianie — w odróżnieniu od 10-letniej karty pobytu stałego. Opłata skarbowa za zezwolenie wynosi 640 zł, plus 100 zł za kartę." },
      { q: "Jak i po ilu latach można otrzymać obywatelstwo polskie?", a: "Istnieją dwie odrębne ścieżki. „Uznanie za obywatela” to decyzja wojewody: zwykle po 3 latach pobytu na podstawie pobytu stałego, przy stabilnym dochodzie i tytule prawnym do mieszkania (2 lata — dla małżonków obywateli Polski, 1 rok — dla osób polskiego pochodzenia); opłata skarbowa wynosi 1000 zł, wymagane jest potwierdzenie znajomości języka na poziomie B1. „Nadanie obywatelstwa” to odrębna, uznaniowa decyzja Prezydenta RP, bez sztywnych terminów ani obowiązkowych kryteriów — możliwa nawet bez spełnienia warunków uznania przez wojewodę; opłata skarbowa za tę decyzję wynosi 1669 zł." },
      { q: "Czym jest karta pobytu CUKR dla obywateli Ukrainy?", a: "Karta pobytu CUKR to zezwolenie na pobyt czasowy na 3 lata dla osób ze statusem UKR (na podstawie ochrony czasowej, a nie statusu uchodźcy), które przyjechały do Polski po 23 lutego 2022. Wnioski składa się elektronicznie w portalu MOS. Koszt to 340 zł opłaty skarbowej plus 100 zł za kartę. Polski własny, ustawowy termin składania wniosków to 4 marca 2027. Unijny ramowy okres ochrony czasowej został osobno przedłużony do 4 marca 2028, ale ta decyzja na poziomie UE nie oznacza automatycznej zmiany polskiego krajowego terminu. Polska może — ale nie musi — przedłużyć swój wewnętrzny termin; aktualne przepisy sprawdzaj na gov.pl/UDSC bliżej daty." },
      ...
      { q: "Czy mogę wyjechać z Polski w trakcie procedury?", a: "Tak, ale z ograniczeniami. Przy pobycie czasowym wyjazd powyżej 6 miesięcy oznacza utratę zezwolenia. Do pobytu stałego liczy się pobyt nieprzerwany — jednorazowo poza Polską nie dłużej niż 6 miesięcy, a łącznie nie więcej niż 10 miesięcy. Osobno dla osób ze statusem UKR (ochrona czasowa): wyjazd z Polski na dłużej niż 30 dni jest samodzielną podstawą wygaśnięcia ochrony czasowej — to znacznie krótszy limit niż dla posiadaczy zezwoleń na pobyt." },
