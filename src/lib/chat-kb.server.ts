...
const MAX_LEGAL_CHARS = 9000;
const MAX_LEGAL_BYTES = 6000;
...
export function buildSystemPrompt(query = ""): string {
  return [
    ...
    "You also have access to live web search, but it is restricted to these official sources only: gov.pl and its subdomains (including mos.cudzoziemcy.gov.pl), migrant.wsc.mazowieckie.pl (especially the /komunikaty announcements page, for current notices and queue/processing updates), and isap.sejm.gov.pl (the official Sejm database of legal acts, for verifying exact current law text). Use it when the knowledge base above doesn't cover the question, or the user asks about something time-sensitive like recent announcements or current processing times. When you rely on a live search result, briefly mention which official site it came from.",
